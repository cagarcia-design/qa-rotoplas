...
**Specs nuevos (en `tests/`, ruta aplanada en s25):**
- `3-money-path-purchase.smoke.spec.js` (`@smoke @auth @purchase`) — **cierra el #1 del diagnóstico de profundidad**: la ruta del dinero ahora completa la compra. Reusa `scripts/crear-orden-b2c.js` (login tecleado→checkout→Pagar) → asercióna nº de orden válido `\d{7}[A-Z0-9]{3,}`. Orden creada **`6182026A9KJ5`** + correo de confirmación verificado por Gmail MCP (21:00:20Z). QA-only (crea orden real).
- `1-login.smoke.spec.js` (`@smoke @login`) — login válido → sesión establecida (redirige fuera de /login + header no anónimo). QA-only.
- `4-forms-email.smoke.spec.js` (`@forms @smoke @email`) — submit + **el correo SALE**. Forgot-password → reset verificado por Gmail MCP. Patrón **Modo A**: el spec emite `@@EMAIL_EXPECT {to,subjectRe,sinceTs}` y el agente/dashboard confirma el arribo (Node no llama al MCP). Mutantes signup/contacto/newsletter como `test.fixme` QA-only (pendiente definir email desechable).
- `2-pci-baseline.contract.spec.js` (`@contract @auth`) — guard expected-fail de **BUG-119**: el detalle de pedido expone `**********424242` (6 dígitos del PAN, viola PCI-DSS). Invariante deseado: ningún texto matchea `/\*{2,}\d{5,}/`. Si lo arreglan → unexpected-pass → cerrar bug.
- `1-catalog.contract.spec.js` (`@contract`) — N1: las 7 categorías **listan productos** (`article[class*="card-product-filter"]`, NO `a[href*="/product/"]` — las cards navegan por `on:click`, BUG-147) + PDP alcanzable **dinámicamente** (click en `.galery` de la 1ª card) → reduce dependencia del SKU fijo 310002.
- `5-mobile.contract.spec.js` (`@contract @mobile`) — viewport 375px. **APARCADO (`test.skip`)** tras diagnóstico: el check de overflow daba falso positivo por un drawer off-canvas (`.drawer-panel`; el body tiene `overflow-x:hidden` → `scrollableX=0`, no hay scroll real) y la hamburguesa `label[for="open-menu"]` reporta **width 0** (el icono visible es el `<span><svg>` interno → `toBeVisible` falla). Reactivar tras ajustar (notas en el spec).

**N1 agregado a `1-forms`:** login email malformado, login credenciales inválidas NO autentican (submit real con email-cebo → sigue anónimo), forgot email malformado. Prod-safe.

**Fix:** `2-customer.contract.spec.js` no cargaba el `storageState` (a diferencia de `2-money-path`) → corría anónimo → 3 tests en skip PERMANENTE. Agregado `test.use({ storageState })` → ahora corre logueado (3/3). **Generado `rotoplas-auth-b2c.json`** (`npm run auth:b2c`).

**Arquitectura (decisión):** split **run-rápido sin efectos** (`check:b2c` = `--grep-invert "@capa2|@smoke"`) vs **on-demand mutante** (`check:b2c:{purchase,login,forms:email}`). Tags: `@health @content @contract @auth @forms @smoke @login @purchase @email @capa2 @mobile`. Headers `tests/contracts/b2c/` → `tests/` corregidos (20 archivos).

**Hallazgo (prod):** el forgot-password en **prod NO verifica con `c.agarcia`** — no es cliente del ambiente prod → el "éxito" de UI es anti-enumeración, no envío (confirmado: el reset de QA llegó, el de prod no). Para verificar correo en prod hace falta una cuenta REGISTRADA en prod cuyo inbox leamos.

**Robustez (#3 cerrado):** quitada la PDP de SKU fijo (310002) de `HEALTH_URLS` → health+content ya no dan falso-rojo si el catálogo da de baja ese SKU (ahora **15 URLs** c/u). `1-catalog` cubre la PDP dinámicamente; `1-pdp` conserva el SKU fijo a propósito (cobertura sembrada → precio+habilitado deterministas).

**Dashboard (#5 cerrado):** `scripts/dashboard.js` con **3 botones on-demand** (Compra E2E→orden · Login→sesión · Forms+correo; los 2 primeros `PROD_BLOCKED`) + **display de `@@EMAIL_EXPECT`** (`showEmailExpect`: dice a quién/qué asunto/desde cuándo verificar) + `1-catalog`/`2-pci-baseline` en las áreas. Verificado server-side (:4600) y en el HTML servido. ⚠️ **Reiniciar el panel** para tomar cambios (carga el HTML en memoria al arrancar).

**Docs:** se probó migrar este overview monolítico a `docs/` estructurado y se **REVIRTIÓ** (resúmenes con pérdida — el commit borraba 1618 líneas y agregaba 525). Este monolito es la fuente de verdad; la migración queda en git `6c9fc47` por si se retoma con un paso de verificación.

**Verificación:** suite no-mutante = **verde** (specs afectados re-corridos: health 15 + content 15 + catálogo 8 = 38 passed; el resto sin cambios). Compra E2E QA = orden `6182026A9KJ5`. Prod read-only (`check:b2c:anon`) = **68 passed**, baseline BUG-015/003/001 sostenido. Móvil = `skipped` (no contamina).

**Hecho:** #1 compra E2E · #3 SKU fijo fuera de health/content · #4 validación prod · #5 dashboard · #7 catálogo N1 · #8 login N2 · #9 guard PCI · #10 baseline prod · #12 docs (revertido).

**Pendientes reales:** (1) **N2 forms mutantes** (signup/contacto/newsletter QA-only) — esperando definir email desechable; (2) **contracts móviles 375px** — aparcados con diagnóstico (quitar `test.skip` tras ajustar overflow+hamburguesa); (3) **commit** (todo untracked en `main`).
